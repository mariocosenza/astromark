...
export const TeacherDashboard = () => {
    ...
    const menuItems = [
        ...
        { img: classroom, label: "Classi", color: theme.palette.primary.main },
                ...
                <Stack direction="row" spacing={6}>
                    {menuItems.slice(0, 2).map((item, index) => (
                        <Box
                            ...
                            flexDirection="column"
                            alignItems="center"
                            onClick={() => item.label === "Orario" ? navigation("/teacher/dashboard") : navigation("/teacher/classi")}
                        >
                            <img
                                src={item.img}
                                alt={`${item.label} icon`}
                ...
                <Stack direction="row" spacing={6}>
                    {menuItems.slice(2).map((item, index) => (
                        <Box
                            ...
                            flexDirection="column"
                            alignItems="center"
                            onClick={() => item.label === "Ticket" ? navigation("/teacher/ticket") : navigation("/teacher/impostazioni")}
                        >
                            <img
                                src={item.img}
                                alt={`${item.label} icon`}
